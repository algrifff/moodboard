...
- fonts: 1–4 entries describing the typefaces this brand will use. Trust order, applied STRICTLY in this priority:
  ...
  (3) Explicit font name references in text content or PDF excerpts. If any sticky note, text label, or PDF excerpt explicitly names a typeface ("set in Akzidenz Grotesk", "use Helvetica Neue", "Times New Roman headings", etc.), treat that as a deliberate typography reference and copy the typeface name verbatim into a fonts[] entry. These count even when the surrounding text is mostly informational.
  (4) LOWEST PRIORITY — typography inferable only from photography where you can't name the typeface. Leave \`name\` empty and describe via \`category\` only.

  IGNORE these signals when picking fonts:
  - Text-node "[font: …]" tags whose value is a CSS fallback stack ("ui-sans-serif", "system-ui", "BlinkMacSystemFont", "-apple-system", "Segoe UI", a comma-separated list of generic stacks). Those are editor defaults — the user didn't pick them. Only count a text-node font tag if the value is a recognisable single typeface name.
  - Layout typography in PDFs (the font the PDF itself was typeset in). PDFs are usually reference material; their typesetting is incidental to the brand's chosen fonts. The exception is rule (3): a PDF that explicitly names a typeface.

  \`category\` is the typographic family (neo-grotesque, transitional serif, didone, slab, geometric sans, humanist sans, blackletter, script, etc.). \`role\` is display / subhead / body / caption based on size + intent. \`sample\` is a phrase pulled from the canvas's text content; if there is none, write a single short evocative phrase the brand might use. Empty array only when there is no typographic content of any kind on the canvas.
