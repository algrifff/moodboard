...
describe('proximityGroups', () => {
  ...
  it('exposes a canonical proximity threshold (48px world space)', () => {
    // CLAUDE.md documents this number — if the constant changes, update
    // CLAUDE.md and the rule callouts at the same time.
    expect(GROUP_PROXIMITY_PX).toBe(48)
  })

  it('uses the canonical threshold when no explicit value is passed', () => {
    // 100×100 square at (0,0); second square at (148,0) leaves a 48-px
    // gap — exactly at the threshold, should group.
    expect(proximityGroups([square('a', 0, 0), square('b', 148, 0)])).toEqual([['a', 'b']])
    // 149-px offset = 49-px gap, just past the threshold, should NOT group.
    expect(proximityGroups([square('a', 0, 0), square('b', 149, 0)])).toEqual([])
  })
