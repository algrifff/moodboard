// Brand logos for external source providers. Inlined SVG so they're part of
// the bundle (no extra request) and themable via CSS custom properties.
//
// Notion's logo is monochrome — we tint the "page" with the surrounding card
// background and the "marks" with the current text colour, so the logo
// reads naturally in both light and dark themes.
//
// Google Drive's logo is multi-coloured (Google brand palette). We render
// the brand colours as-is since they work against any neutral background.
...
export function DriveLogo({ size = 14 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 87.3 78" style={{ flexShrink: 0 }} aria-hidden>
      <path
        fill="#0066da"
